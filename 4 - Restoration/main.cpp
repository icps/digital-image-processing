...
// http://vgg.fiit.stuba.sk/2012-05/frequency-domain-filtration-source/
...
void updateMag(Mat complexI )
{
    ...
    Mat magI;
    Mat planes[] = {Mat_<float>(complexI), Mat::zeros(complexI.size(), CV_32F)};//{Mat::zeros(complex.size(), CV_32F), Mat::zeros(complex.size(), CV_32F)};
    ...
    magI = magI(cv::Rect(0, 0, magI.cols & -2, magI.rows & -2)); // add
...
Mat createGausFilterMask(Size mask_size, int x, int y, int ksize = 5) {
...
void onMouse( int event, int x, int y, int, void* param)
{
    ...
    Mat* p_complex = static_cast<Mat*> (param);
    Mat complex = *p_complex;

    Mat mask = createGausFilterMask(complex.size(), x, y, 50);//kernel_size, true, true);

    // show the kernel
    namedWindow("Gaussian Mask", CV_WINDOW_NORMAL);
    imshow("Gaussian Mask", mask);

    shift(mask);

    Mat planes[] = {Mat_<float>(complex), Mat::zeros(complex.size(), CV_32F)};//{Mat::zeros(complex.size(), CV_32F), Mat::zeros(complex.size(), CV_32F)};
    Mat kernel_spec;
    planes[0] = mask; // real
...
int main()
{
    Mat image = imread("/home/isadora/Imagens/PDI/q08.tif", CV_LOAD_IMAGE_GRAYSCALE);
    namedWindow( "Orginal window", CV_WINDOW_NORMAL);// Create a window for display.
